// NOT.ESP QUIZ — v16 (shuffle-bag coverage)
const DEFAULT_DATA = {
  "🔴":"wonderful","➕":"towards","🌊":"free","🔺":"fair choice","🟨":"fantastic",
  "⭐":"finally","🧍‍♂️":"human","😀":"zoomed","❤️":"affectionate","🚤":"serene",
  "🏠":"wholesome","🚴‍♂️":"movement","🚗":"speed","🚂":"momentum","✈️":"take flight",
  "🚀":"outer space","⛩":"surface","📺":"visual","☎️":"communicate","📖":"literature",
  "🍷":"drink from","✏️":"artistic","🔐":"security","⏰":"keeping time","🔪":"sharp",
  "🔫":"lethal","🌳":"natural","🌻":"organic","☀️":"painting","🌙":"night sky",
  "🌧️":"weather","⚡":"powerful","🐶":"loyal","🐱":"living","🐟":"underwater",
  "🦜":"in the sky","👽":"weirdly","👻":"spooky","💀":"morbid","📝":"nothing",
  "🖕":"offensive","🚫":"negative","🍆":"strangely"
};
const LS_DATA='notesp-quiz'; const LS_STATE='notesp-quiz-state-v3'; const LS_MODE='notesp-quiz-mode-v4';
...
function loadState(d){const base=Object.keys(d).map(k=>({emoji:k,word:d[k],box:1,seen:0,correctStreak:0,lastSeen:-1}));try{const s=JSON.parse(localStorage.getItem(LS_STATE));if(!s)return base;const m=new Map(base.map(i=>[i.emoji,i]));for(const it of s)if(m.has(it.emoji))Object.assign(m.get(it.emoji),it);return[...m.values()]}catch{return base}}
...
let lastIndex=-1, qn=0, correct=0, wrong=0, streak=0;
let awaitingNext=false, nextBtnEl=null, nextKeyHandler=null;
let cur=-1, rightEmoji=null, rightWord=null;

// ---- shuffle-bag to guarantee coverage ----
let bag=[];
function shuffle(a){for(let i=a.length-1;i>0;i--){const j=Math.floor(Math.random()*(i+1));[a[i],a[j]]=[a[j],a[i]]}return a}
function refillBag(){
  bag = shuffle([...Array(items.length).keys()]);
  // avoid immediate repeat if possible
  if(bag.length>1 && bag[0]===lastIndex){ [bag[0], bag[1]] = [bag[1], bag[0]]; }
}
function nextIndex(){
...
function hud(){const t=correct+wrong; el.ok.textContent=correct; el.ko.textContent=wrong; el.acc.textContent=t?Math.round((correct/t)*100)+'%':'0%'; el.streak.textContent=streak}
function toast(msg){el.toast.textContent=msg; el.toast.classList.add('show'); setTimeout(()=>el.toast.classList.remove('show'),1100)}
...
function showResult(ok,typed,correctWord){el.resultBanner.className='result-banner show '+(ok?'ok':'no'); el.resultBanner.innerHTML=ok?`✅ Correct: <strong>${correctWord}</strong>`:`❌ Wrong: <em>${typed||'—'}</em> → Correct: <strong>${correctWord}</strong>`}
...
function render(){
  ...
  if(mode===MODES.W2E_MCQ){
    const pool=items.map(x=>x.emoji), count=Math.min(12,pool.length); const set=new Set([rightEmoji]); while(set.size<count)set.add(pool[Math.floor(Math.random()*pool.length)]);
    ...
    const pool=items.map(x=>x.word), count=Math.min(10,pool.length); const set=new Set([rightWord]); while(set.size<count)set.add(pool[Math.floor(Math.random()*pool.length)]);
  ...
  lastIndex = cur; // track after render so refill can avoid immediate repeat
}
...
function answer(sel){
  ...
  saveState(); hud(); setTimeout(render,900);
}
...
function onReveal(){
  ...
  if(ok){ correct++; streak++; it.correctStreak++; if(it.box<5 && it.correctStreak>=2){it.box++; it.correctStreak=0} showResult(true,typedRaw,rightWord); saveState(); hud(); el.overlay.classList.add('hidden'); setTimeout(render,1100);
...
const EMOJI_PALETTE=[ "🔴","🟠","🟡","🟢","🔵","🟣","🟤","⚫","⚪","🔺","🔻","⬜","⬛","🟥","🟧","🟨","🟩","🟦","🟪","🟫","⭐","✨","🔥","⚡","❄️","💧","☔","🌪️","🌈","☀️","🌙","🌧️","⛄","😀","😃","😄","😁","😆","😊","🙂","😉","😍","😎","🤔","😴","🤯","🥳","🧍‍♂️","🧍‍♀️","🚶‍♂️","🏃‍♂️","🚴‍♂️","🚴‍♀️","🧗‍♂️","🏊‍♂️","❤️","💛","💚","💙","💜","🖤","🤍","🤎","🚗","🚕","🚙","🚌","🚎","🏎️","🚓","🚑","🚒","🚐","🚚","🚛","🚜","🚂","🚆","✈️","🚀","🚤","🛶","🚁","🛸","🏠","🏢","🏫","🏛️","⛩","🏯","🏰","📺","☎️","📱","💻","⌚","⏰","🔐","🔒","🔑","✏️","📖","🍎","🍇","🍔","🍕","🍷","🍺","☕","🍩","🌳","🌲","🌴","🌻","🌸","🍀","🐶","🐱","🐟","🦜","🐘","🦁","👽","👻","💀","📝","🚫","🖕","🍆" ];
function buildEmojiSet(){const set=new Set(EMOJI_PALETTE); items.forEach(i=>set.add(i.emoji)); return[...set]}
function emojiExists(em, exceptRow=null){const rows=[...document.querySelectorAll('#editorList .editor-item')]; for(const r of rows){ if(r===exceptRow) continue; const t=r.querySelector('.editor-emoji')?.textContent.trim(); if(t===em) return true } return false }
...
document.addEventListener('input',(e)=>{
  if(e.target && e.target.id==='editorSearch'){ e.stopPropagation(); renderEditorList(); }
});
...
document.addEventListener('change',(e)=>{
  if(e.target && e.target.id==='editorImportFile'){ const f=e.target.files[0]; if(!f) return; const r=new FileReader(); r.onload=()=>{ try{ const obj=JSON.parse(r.result); localStorage.setItem(LS_DATA, JSON.stringify(obj)); data=loadData(); items=loadState(data); bag=[]; renderEditorList(); toast('Imported'); } catch{ alert('Invalid JSON'); } }; r.readAsText(f); e.target.value=''; }
});
